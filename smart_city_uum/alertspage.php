...
<style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Outfit', sans-serif; }
    ...
    .glass-panel {
        ...
        border: 1px solid var(--card-border); box-shadow: var(--shadow); border-radius: 24px;
    }
    ...
    .main { flex: 1; padding: 12px 18px 14px; display: flex; flex-direction: column; height: 100vh; min-width: 0; overflow: hidden; transition: padding 0.4s; }

    .topbar { display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px; flex-shrink: 0; }
    ...
    /* MAP PANEL (flex:1 like weather map-panel) */
    .map-panel {
        flex: 1;
    ...
    #alertMap { width: 100%; height: 100%; z-index: 1; }
    .map-stats {
        position: absolute;
        bottom: 15px;
        left: 15px;
    ...
    .legend-title { font-size: 10px; text-transform: uppercase; letter-spacing: 0.5px; color: var(--muted); margin-bottom: 8px; font-weight: 600; }
    .legend-item { display: flex; align-items: center; gap: 8px; margin-bottom: 5px; }
    .legend-item:last-child { margin-bottom: 0; }
    .legend-diamond { width: 10px; height: 10px; border-radius: 3px; transform: rotate(45deg); }
    .legend-diamond.responding { background: var(--success); box-shadow: 0 0 8px rgba(0, 255, 135, 0.5); animation: legendPulse 2s infinite; }
    .legend-diamond.patrolling { background: var(--primary); box-shadow: 0 0 6px rgba(0, 242, 254, 0.4); }
    .legend-diamond.onscene { background: var(--warning); box-shadow: 0 0 6px rgba(254, 193, 99, 0.4); }
    @keyframes legendPulse { 0%, 100% { opacity: 1; } 50% { opacity: 0.6; } }

    /* CCTV SIDE PANEL (fixed width like nearby-panel) */
    .cctv-side-panel {
        width: 278px;
        display: flex;
        flex-direction: column;
    ...
    /* CCTV SURVEILLANCE PANEL inside side panel */
    .cctv-surveillance {
        display: flex;
        flex-direction: column;
        overflow: hidden;
    ...
    .cctv-header h3 { font-size: 13px; margin: 0; display: flex; align-items: center; gap: 8px; }
    ...
    .cctv-grid::-webkit-scrollbar { width: 4px; }
    ...
    .cctv-monitor video { width: 100%; height: 100%; object-fit: cover; filter: grayscale(60%) contrast(130%) brightness(60%) sepia(10%) hue-rotate(180deg); }
    ...
    .recording-badge {
        position: absolute; top: 4px; right: 4px; background: rgba(255, 0, 0, 0.8); color: white; padding: 1px 6px;
        border-radius: 3px; font-size: 8px; font-weight: 700; display: flex; align-items: center; gap: 3px;
    }
    .rec-dot { width: 5px; height: 5px; background: white; border-radius: 50%; animation: blink 1s infinite; }
    .area-label {
        position: absolute; bottom: 4px; left: 4px; background: rgba(0, 0, 0, 0.75); backdrop-filter: blur(4px);
        color: #00ff87; padding: 1px 6px; border-radius: 3px; font-size: 8px; font-weight: 600; text-transform: uppercase;
        letter-spacing: 0.3px; border: 1px solid rgba(0, 255, 135, 0.3); z-index: 2;
    }
    .cctv-info { padding: 7px 8px; }
    .cctv-label { font-size: 10px; color: var(--muted); margin-bottom: 1px; }
    .cctv-location { font-size: 11px; font-weight: 600; color: var(--text); margin-bottom: 2px; }
    .cctv-event { font-size: 9px; padding: 2px 6px; border-radius: 3px; display: inline-block; }
    .cctv-event.active { background: rgba(255, 0, 64, 0.2); color: #ff4d79; border: 1px solid rgba(255, 0, 64, 0.3); }
    .cctv-event.normal { background: rgba(0, 255, 135, 0.1); color: var(--success); border: 1px solid rgba(0, 255, 135, 0.2); }

    /* Expanded state for CCTV */
    .cctv-surveillance.expanded .cctv-grid {
        position: absolute; top: 0; left: 0; right: 0; bottom: 0;
        z-index: 100; padding: 0; margin: 0; border-radius: 22px; overflow: hidden;
        background: #000; border: 2px solid #00ff87;
    ...
    .cctv-surveillance.expanded .cctv-monitor.cctv-expanded {
        ...
        box-shadow: none; animation: none; background: #000; margin: 0;
    }
    ...
    .cctv-surveillance.expanded .cctv-monitor.cctv-expanded .cctv-label { font-size: 14px; color: #fff; }
    .cctv-surveillance.expanded .cctv-monitor.cctv-expanded .cctv-location { font-size: 18px; color: #00ff87; }
    .cctv-surveillance.expanded .cctv-monitor.cctv-expanded .cctv-event { font-size: 13px; padding: 5px 14px; }
    .cctv-surveillance.expanded .cctv-monitor.cctv-expanded .area-label { font-size: 14px; padding: 5px 12px; bottom: 80px; left: 15px; }
    .cctv-surveillance.expanded .cctv-monitor.cctv-expanded .recording-badge { font-size: 12px; padding: 4px 12px; top: 15px; right: 15px; }
    @keyframes cctvPulse { 0%, 100% { box-shadow: 0 0 30px rgba(0, 255, 135, 0.4); } 50% { box-shadow: 0 0 50px rgba(0, 255, 135, 0.6); } }

    .cctv-expand-close {
        position: absolute; top: 15px; right: 15px; z-index: 101;
        background: rgba(255, 75, 43, 0.85); border: 2px solid rgba(255, 75, 43, 0.6);
        color: white; width: 40px; height: 40px; border-radius: 50%;
        display: none; align-items: center; justify-content: center; cursor: pointer;
        font-size: 18px; transition: all 0.3s ease; box-shadow: 0 0 15px rgba(255, 75, 43, 0.4);
    }
    .cctv-expand-close:hover { background: rgba(255, 75, 43, 1); transform: scale(1.1); box-shadow: 0 0 25px rgba(255, 75, 43, 0.6); }
    .cctv-surveillance.expanded .cctv-expand-close { display: flex; }

    /* BOTTOM ROW - Alerts + Patrol Units */
    .alerts-bottom-row {
        display: flex;
        gap: 12px;
        flex-shrink: 0;
    ...
    .alerts-header h3 { font-size: 13px; margin: 0; display: flex; align-items: center; gap: 8px; }
    .alerts-list {
        overflow-y: auto;
        flex: 1;
        padding: 8px;
    ...
    .alert-card.alert-cam-highlight::before { width: 6px; background: #00ff87 !important; box-shadow: 0 0 20px rgba(0, 255, 135, 0.8) !important; }
    .alert-card.alert-cam-highlight .alert-location { color: #00ff87; }
    .alert-card.alert-cam-highlight .alert-desc { color: rgba(255,255,255,0.8); }
    .alert-card.alert-cam-highlight .alert-id { color: rgba(0, 255, 135, 0.5); }
    ...
    .alert-badge.warning { background: rgba(254, 193, 99, 0.2); color: var(--warning); border: 1px solid rgba(254, 193, 99, 0.4); }
    .alert-badge.notice { background: rgba(0, 242, 254, 0.15); color: var(--primary); border: 1px solid rgba(0, 242, 254, 0.3); }
    .alert-badge.normal { background: rgba(0, 255, 135, 0.15); color: var(--success); border: 1px solid rgba(0, 255, 135, 0.3); }
    @keyframes pulse-critical { 0%, 100% { box-shadow: 0 0 5px rgba(255, 0, 64, 0.3); } 50% { box-shadow: 0 0 20px rgba(255, 0, 64, 0.6); } }
    ...
    .patrol-header h3 { font-size: 13px; margin: 0; display: flex; align-items: center; gap: 8px; }
    .patrol-list {
        overflow-y: auto;
        flex: 1;
        padding: 10px;
    ...
    .patrol-list::-webkit-scrollbar { width: 4px; }
    ...
    .patrol-icon { width: 42px; height: 42px; border-radius: 10px; background: rgba(0, 242, 254, 0.1); display: flex; align-items: center; justify-content: center; font-size: 16px; color: var(--primary); flex-shrink: 0; }
    .patrol-details { flex: 1; }
    .patrol-unit { font-size: 13px; font-weight: 600; color: var(--text); margin-bottom: 1px; }
    .patrol-officers { font-size: 11px; color: var(--muted); margin-bottom: 3px; }
    .patrol-location { font-size: 11px; color: var(--primary); display: flex; align-items: center; gap: 4px; }
    .patrol-chip { padding: 4px 10px; border-radius: 20px; font-size: 10px; font-weight: 600; text-transform: uppercase; text-align: center; }
    .patrol-chip.responding { background: rgba(255, 75, 43, 0.2); color: var(--danger); border: 1px solid rgba(255, 75, 43, 0.4); }
    .patrol-chip.onscene { background: rgba(255, 0, 64, 0.2); color: #ff4d79; border: 1px solid rgba(255, 0, 64, 0.4); }
    .patrol-chip.patrolling { background: rgba(0, 255, 135, 0.15); color: var(--success); border: 1px solid rgba(0, 255, 135, 0.3); }
    .patrol-chip.offline { background: rgba(255, 255, 255, 0.05); color: var(--muted); border: 1px solid rgba(255, 255, 255, 0.1); }

    @media(max-width: 1200px) {
        .alerts-top-row, .alerts-bottom-row { flex-direction: column; height: auto; }
        .cctv-side-panel, .patrol-panel { width: 100%; }
    ...
    <main class="main">
        ...
        <div class="topbar">
            <div class="topbar-left">
                ...
                <div>
                    <h3 style="font-size: 18px; font-weight: 700; margin: 0;">Security Alerts</h3>
                    <p style="color: var(--muted); margin:0; font-size: 12px;">Crime Surveillance & Alarm Monitoring</p>
                </div>
            </div>
        ...
        <div class="alerts-grid">
            ...
            <!-- TOP ROW: Map + CCTV -->
            <div class="alerts-top-row">

                <!-- MAP PANEL -->
                <div class="map-panel">
                    <div class="map-panel-header">
                        <div class="section-title mb-0">
                            <span><i class="fa-solid fa-map-location-dot text-primary me-2"></i>Incident Map - Changlun</span>
                        </div>
                    </div>
                    <div class="map-canvas">
                        <div id="alertMap"></div>
                        <div class="map-stats">
                            <div class="stat-item"><span class="stat-dot critical"></span><span>Critical: <strong id="criticalCount">0</strong></span></div>
                            <div class="stat-item"><span class="stat-dot warning"></span><span>Warnings: <strong id="warningCount">0</strong></span></div>
                            <div class="stat-item"><span class="stat-dot patrol"></span><span>Patrols: <strong id="patrolCount">0</strong></span></div>
                        </div>
                        <div class="map-legend">
                            <div class="legend-title"><i class="fa-solid fa-car-side me-1"></i> Patrol Status</div>
                            <div class="legend-item"><div class="legend-diamond responding"></div><span>Responding</span></div>
                            <div class="legend-item"><div class="legend-diamond patrolling"></div><span>Patrolling</span></div>
                            <div class="legend-item"><div class="legend-diamond onscene"></div><span>On Scene</span></div>
                        </div>
                    </div>
                </div>

                <!-- CCTV SIDE PANEL -->
                <div class="cctv-side-panel">
                    <div class="cctv-surveillance" id="cctvSurveillance">
                        <div class="cctv-header">
                            <h3><i class="fa-solid fa-video text-success me-2"></i>Live Surveillance</h3>
                        </div>
                        <button class="cctv-expand-close" id="cctvCloseBtn" onclick="collapseCctv()">
                ...
                <div class="alerts-panel">
                    ...
                    <div class="alerts-list">
                        <?php foreach ($crimeAlerts as $alert):
                            $alertCamIndex = -1;
                            if (stripos($alert['location'], 'Maybank') !== false) $alertCamIndex = 0;
                            elseif (stripos($alert['location'], '99 Speedmart') !== false) $alertCamIndex = 1;
                            elseif (stripos($alert['location'], 'Public Bank') !== false) $alertCamIndex = 2;
                            elseif (stripos($alert['location'], 'Petronas') !== false) $alertCamIndex = 3;
                        ?>
                        <div class="alert-card <?php echo strtolower($alert['status']); ?>" data-lat="<?php echo $alert['lat']; ?>" data-lng="<?php echo $alert['lng']; ?>" data-cam="<?php echo $alertCamIndex; ?>">
                            <div class="alert-card-header">
                                ...
                                <span class="<?php echo alertBadgeClass($alert['status']); ?>"><?php echo htmlspecialchars($alert['status']); ?></span>
                            </div>
                    ...
                    <div class="patrol-section">
                        ...
                        <div class="patrol-list">
                            ...
                            <div class="patrol-card">
                                <div class="patrol-icon"><i class="fa-solid fa-car-side"></i></div>
                                ...
                                <span class="patrol-chip <?php echo strtolower(str_replace(' ', '', $unit['status'])); ?>">
                                    ...
                                    <?php if ($unit['eta'] !== '-'): ?><br><small style="font-size: 8px;">ETA: <?php echo htmlspecialchars($unit['eta']); ?></small><?php endif; ?>
                                </span>
                            </div>
                            <?php endforeach; ?>
                        </div>
...
<script>
    ...
    var criticalIcon = L.divIcon({ className: 'custom-div-icon', html: '<div style="background-color: #ff0040; width: 18px; height: 18px; border-radius: 50%; box-shadow: 0 0 20px 5px rgba(255,0,64, 0.7); border: 3px solid white; animation: pulse 1.5s infinite;"></div>', iconSize: [18, 18], iconAnchor: [9, 9] });
    var warningIcon = L.divIcon({ className: 'custom-div-icon', html: '<div style="background-color: #fec163; width: 14px; height: 14px; border-radius: 50%; box-shadow: 0 0 15px 3px rgba(254,193,99, 0.5); border: 2px solid white;"></div>', iconSize: [14, 14], iconAnchor: [7, 7] });
    var noticeIcon = L.divIcon({ className: 'custom-div-icon', html: '<div style="background-color: #00f2fe; width: 12px; height: 12px; border-radius: 50%; box-shadow: 0 0 10px 2px rgba(0,242,254, 0.5); border: 2px solid white;"></div>', iconSize: [12, 12], iconAnchor: [6, 6] });

    function createPatrolIcon(unitId, isMoving) {
        return L.divIcon({
            className: 'custom-div-icon',
            html: `<div style="background-color: #00ff87; width: 20px; height: 20px; border-radius: 4px; transform: rotate(45deg); box-shadow: 0 0 15px 3px rgba(0,255,135, 0.5); border: 2px solid white; ${isMoving ? 'animation: patrolPulse 2s infinite;' : ''}"></div>
            <div style="position: absolute; top: -2px; left: 50%; transform: translateX(-50%); font-size: 8px; font-weight: bold; color: #00ff87; text-shadow: 0 0 4px #000; background: rgba(0,0,0,0.7); padding: 1px 3px; border-radius: 3px;">${unitId}</div>`,
            iconSize: [24, 24], iconAnchor: [12, 12]
        });
    }

    var style = document.createElement('style');
    style.innerHTML = `@keyframes pulse { 0% { transform: scale(1); box-shadow: 0 0 20px 5px rgba(255,0,64, 0.7); } 50% { transform: scale(1.3); box-shadow: 0 0 30px 10px rgba(255,0,64, 0.4); } 100% { transform: scale(1); box-shadow: 0 0 20px 5px rgba(255,0,64, 0.7); } } @keyframes patrolPulse { 0%, 100% { box-shadow: 0 0 15px 3px rgba(0,255,135, 0.5); } 50% { box-shadow: 0 0 25px 8px rgba(0,255,135, 0.8); } }`;
    document.head.appendChild(style);

    var alertCameraMap = { 'Maybank ATM': 0, '99 Speedmart': 1, 'Public Bank': 2, 'Petronas Station': 3 };

    var alertData = <?php echo json_encode($crimeAlerts); ?>;
    var criticalCount = 0, warningCount = 0;

    alertData.forEach(function(alert) {
        var icon; var status = alert.status.toLowerCase();
        ...
        var camIndex = -1;
        for (var key in alertCameraMap) { if (alert.location.indexOf(key) !== -1) { camIndex = alertCameraMap[key]; break; } }

        marker.bindPopup(`<div style="color: #333; min-width: 200px;"><strong style="color: ${status === 'critical' ? '#ff0040' : '#fec163'};">${alert.type}</strong><br><span style="color: #666;">${alert.location}</span><br><small style="color: #999;">${alert.description}</small><br><span style="color: #999; font-size: 11px;">${alert.time}</span>${camIndex !== -1 ? '<br><small style="color: #00ff87;">Click to expand CCTV</small>' : ''}</div>`);

        marker.on('click', function() { if (camIndex !== -1) expandCamera(camIndex); });
    });
    ...
    function expandCamera(index) {
        ...
        if (monitors[index]) {
            monitors[index].classList.add('cctv-expanded');
    ...
    var patrolRoutes = {
        'PV-01': { waypoints: [[6.4330, 100.4290],[6.4350, 100.4310],[6.4380, 100.4280],[6.4360, 100.4250],[6.4330, 100.4270],[6.4330, 100.4290]], speed: 0.00005, currentWaypoint: 0, status: 'responding' },
        'PV-02': { waypoints: [[6.4360, 100.4250],[6.4400, 100.4220],[6.4380, 100.4180],[6.4340, 100.4200],[6.4360, 100.4250]], speed: 0.000035, currentWaypoint: 0, status: 'patrolling' },
        'PV-03': { waypoints: [[6.4290, 100.4315],[6.4285, 100.4310],[6.4300, 100.4330],[6.4280, 100.4350],[6.4290, 100.4315]], speed: 0.00002, currentWaypoint: 0, status: 'onscene' }
    };

    var patrolMarkers = {}, patrolTrails = {};

    Object.keys(patrolRoutes).forEach(function(unitId) {
        var route = patrolRoutes[unitId]; var startPos = route.waypoints[0];
        var marker = L.marker(startPos, { icon: createPatrolIcon(unitId, route.status !== 'onscene') }).addTo(map);
        marker.bindPopup(`<div style="color: #333; min-width: 180px;"><strong style="color: #00ff87;">${unitId}</strong><br><span style="color: #666; font-size: 12px;">Patrol Unit</span><br><span style="color: #999; font-size: 11px;">Status: ${route.status.toUpperCase()}</span></div>`);
        patrolMarkers[unitId] = marker;
        var trail = L.polyline([startPos], { color: '#00ff87', weight: 2, opacity: 0.3, dashArray: '5, 10' }).addTo(map);
        patrolTrails[unitId] = trail;
    });

    var animationFrame, lastUpdate = Date.now();
    ...
    function animatePatrols() {
        ...
        Object.keys(patrolRoutes).forEach(function(unitId) {
            var route = patrolRoutes[unitId], marker = patrolMarkers[unitId], trail = patrolTrails[unitId];
            if (route.status === 'onscene') {
                if (Math.random() < 0.01) { var cp = marker.getLatLng(); var off = [(Math.random() - 0.5) * 0.0001, (Math.random() - 0.5) * 0.0001]; marker.setLatLng([cp.lat + off[0], cp.lng + off[1]]); }
                return;
            }
            var cp = route.waypoints[route.currentWaypoint], nw = route.waypoints[(route.currentWaypoint + 1) % route.waypoints.length], dist = distance(cp, nw);
            if (dist < 0.00001) { route.currentWaypoint = (route.currentWaypoint + 1) % route.waypoints.length; return; }
            var ma = route.speed * deltaTime * 60, t = Math.min(ma / dist, 1), np = interpolate(cp, nw, t);
            marker.setLatLng(np);
